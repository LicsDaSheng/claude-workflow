...
**需要注意的词汇：** 是……不是……、不是……而是……、不做……而是……、不负责……负责……、不接收……只接收……

**问题：**
这种句式本身没有错，很多技术文档、产品说明和分析文章都需要它来划边界。问题在于一旦高频重复，文本会出现明显的模板感：

* 节奏单一，像在反复套同一种句型
* 解释感过强，像在不断纠正读者
* 信息表面上很多，但不少内容其实可以直接正向定义
* 连续几段都这么写时，文章会越来越像讲解稿，而不像成熟文本
...
**什么时候应该改写：**
如果一句话的重点其实是“它负责什么”，而不是“它不是什么”，就应改成正向定义。

**改写前：**
> 这一层不是展示层，而是任务入口层。

**改写后：**
> 这一层是任务入口层，负责整理输入来源和任务上下文。
...
* 能直接定义职责，就不要先绕一句否定
* 每个大节保留 1 次必要的反向澄清就够了
* 连续两段以上出现同类句式时，至少改掉一半
* 优先改成“负责……”“输入是……”“输出是……”“边界在……”这类正向表述
